...
const ProductList = ({ selectedCategory, user }) => {
  const { comments } = useComments(); // Access comments from context
  const products = [
          ...
          {filteredProducts.map((product) => (
            <div key={product._id} className={`product-card ${isAdded === product._id ? 'added-to-cart' : ''}`}>
              ...
              <div className="product-details">
                ...
                {comments[product._id] && comments[product._id].length > 0 && (
                  <p>{comments[product._id].length} Comment{comments[product._id].length > 1 ? 's' : ''}</p>
                )}
